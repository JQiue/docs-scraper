...
class DocumentationSpider(CrawlSpider, SitemapSpider):
    ...
    def __init__(self, config, meilisearch_helper, strategy, *args, **kwargs):
        ...
        self.only_urls = config.only_urls
        ...
        allow_regex = self.only_urls if self.only_urls else start_urls_any_scheme
        ...
        if self.only_urls:
            DocumentationSpider.rules = [
                Rule(link_extractor, callback=self.parse_from_start_url, follow=False),
            ]
        else:
            DocumentationSpider.rules = [
