...
class MeiliSearchHelper:
    ...
    def __init__(self, host_url, api_key, index_uid, custom_settings, is_update):
        self.meilisearch_client = meilisearch.Client(
            host_url, api_key, client_agents=(qualified_version(),)
        )
    ...
    def get_document_id_by_query(self, query):
        result = self.meilisearch_index.search(query)
        lists = []
        for item in result["hits"]:
            if item["url"].startswith(query):
                lists.append(item["objectID"])
        return lists
